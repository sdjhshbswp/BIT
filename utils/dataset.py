...
class RecDataSet(BaseDataSet):
    def __init__(self, train, user_vocab=None, is_wx=False) -> None:
        super().__init__()
        prefix = ""
        if is_wx:
            prefix = "Weixin"
        if train == 'train':
            self.sampler = getattr(utils, f"{prefix}Sampler")(data_path=const.rec_train,
                                   search=False,
                                   user_vocab=user_vocab)
        elif train == 'val':
            self.sampler = getattr(utils, f"{prefix}Sampler")(data_path=const.rec_val,
                                   search=False,
                                   user_vocab=user_vocab)
        elif train == 'test':
            self.sampler = getattr(utils, f"{prefix}Sampler")(data_path=const.rec_test,
                                   search=False,
                                   user_vocab=user_vocab)
...
class SrcDataSet(BaseDataSet):
    def __init__(self, train, user_vocab=None, is_wx=False) -> None:
        ...
        if is_wx:
            prefix = "Weixin"
        if train == 'train':
            self.sampler = getattr(utils, f"{prefix}Sampler")(data_path=const.src_train,
                                   search=True,
                                   user_vocab=user_vocab)
        elif train == 'val':
            self.sampler = getattr(utils, f"{prefix}Sampler")(data_path=const.src_val,
                                   search=True,
                                   user_vocab=user_vocab)
        elif train == 'test':
            self.sampler = getattr(utils, f"{prefix}Sampler")(data_path=const.src_test,
                                   search=True,
                                   user_vocab=user_vocab)


class InfoNCEDataset(Dataset):
    def __init__(self, query_vocab, is_wx=False) -> None:
        super().__init__()
        if is_wx:
            self.query_vocab = np.array(list(range(1, const.word_id_num)))
        else:
            # skip 0 for padding
    ...
    def __getitem__(self, index):
        ...
        if self.is_wx:
            query = self.query_vocab[index % self.query_vocab.size].item()
        else:
