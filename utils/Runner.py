...
from utils.utils import batch_to_gpu, format_metric, load_pickle, worker_init_fn
...
class BaseRunner(object):
    ...
    def __init__(self, args) -> None:
        ...
        self.infoNCE_neg_sample = args.batch_size
    ...
    def train(self, model: BaseModel):
        ...
        if model.query_item_alignment:
            ...
            # else:
            self.InfoNCE_dataloader = self.getDataLoader(
                InfoNCEDataset(query_vocab=(None if self.is_wx else self.query_vocab), is_wx=self.is_wx),
                self.infoNCE_neg_sample,
                shuffle=False)
    ...
    def predict(model: BaseModel, test_loader: DataLoader):
        ...
        for step, batch in enumerate(test_loader):
            prediction = model.predict(batch_to_gpu(batch, model.device))
    ...
    def build_dataset(self):
        if not self.is_wx:
            if self.datasetParaDict['user_vocab'] is None:
                self.datasetParaDict['user_vocab'] = load_pickle(
                    const.user_vocab)

    def get_query_vocab(self):
        if self.query_vocab is None:
            self.query_vocab = load_pickle(const.query_vocab)
...
class SarRunner(BaseRunner):
    ...
    def build_dataset(self):
        ...
        self.traindata = {
            ...
            RecDataSet(train='train',
                       user_vocab=self.datasetParaDict['user_vocab'], is_wx=self.is_wx),
            "src":
    ...
    def train_epoch(self, epoch: int, model: BaseModel):
        ...
        for step, rec_batch in enumerate(tqdm(self.rec_train_loader)):
            ...
            rec_loss = model.loss(batch_to_gpu(rec_batch, model.device))
            src_loss = model.src_loss(
                batch_to_gpu(src_batch, model.device))
    ...
    def evaluate(self, model: BaseModel, mode: str):
        ...
        results = {
            "rec": format_metric(rec_results),
            "src": format_metric(src_results)
        }
