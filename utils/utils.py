...
def format_metric(result_dict: Dict[str, Any], tb_writer=None, tb_tag=None, epoch=0) -> str:
    ...
    for topk in np.sort(topks):
        for metric in np.sort(metrics):
            ...
            m = result_dict[name]
            if type(m) is float or type(m) is float or type(
                    m) is np.float32 or type(m) is np.float64:
                if tb_writer:
                    tb_writer.add_scalar(
                    f"{tb_tag}_{name}:", m, epoch
                )
                format_str.append('{}:{:<.4f}'.format(name, m))
            elif type(m) is int or type(m) is int or type(
