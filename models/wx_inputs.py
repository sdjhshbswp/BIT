...
class ItemFeat(nn.Module):
    ...
    def forward(self, sample):
        feats_ls = []
        result_emb = torch.zeros((sample["item_id"].shape[0] * sample["item_id"].shape[1], const.final_emb_size),
                                    device=sample["item_id"].device)
        for attr in self.attr_ls:
            new_sample = sample[attr].reshape((-1, ))
            
            # sub_mask = new_sample != 0
            # TODO: workaround, keep all 
            # sub_mask = new_sample != -1
            # if sub_mask.sum() > 0:
            ...
            if attr == 'item_id':
                ...
                index = new_sample.reshape(-1, const.max_query_word_len)
            feats_ls.append(getattr(self, f'{attr}_emb')(index))
        # if sub_mask.sum() > 0:
        sub_sample_emb = torch.tanh(
            self.item_trans(torch.cat(feats_ls, dim=-1))).clone()
        # result_emb[sub_mask] = sub_sample_emb
        result_emb = sub_sample_emb
        result_emb = result_emb.reshape((*sample["item_id"].shape, const.final_emb_size))
        return result_emb
...
class QueryEncoder(nn.Module):
    ...
    def forward(self, seqs):
        ...
        # assert seqs_len.any() != 0
        mean_emb = sum_emb / (seqs_len + 1e-6)
...
class HistoryEncoder(nn.Module):
    ...
    def forward(self, seqs):
        ...
        # output = output.masked_fill(seqs_len == 0, 0)

        return output


class HistoryFeat(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.his_encoder = HistoryEncoder()
    ...
    def forward(self, sample: torch.Tensor):
        query_emb: torch.Tensor = self.his_encoder(sample)
        # query_emb = query_emb.reshape((*sample.shape[:-1], -1))
...
class SrcSessionFeat(nn.Module):
    ...
    def get_history_emb(self, sample):
        return self.his_feat(sample)
